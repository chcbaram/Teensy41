...
uint16_t image_buf[160 * 144] __attribute__((section(".lcd_buf")));
...
void ev_poll()
{
  if (buttonGetPressedTime(_PIN_BUTTON_HOME) > 50 && buttonGetPressedEvent(_PIN_BUTTON_HOME) == true)
  {
    vid_change_mode();
  }
}
...
bool osd_menu(void)
{
  ...
  if (buttonGetPressedTime(_PIN_BUTTON_MENU) > 50 && buttonGetPressedEvent(_PIN_BUTTON_MENU) == true)
  {
  }
  else
  {
  ...
  while(1)
  {
    ...
    if (key & (1<<_PIN_BUTTON_DOWN))
    {
      cursor_last = cursor;
      cursor = (cursor + 1) % cursor_max;
    }

    if (key & (1<<_PIN_BUTTON_A))
    {
      if (cursor == 0) // SAVE
      {
        state_save(0);
